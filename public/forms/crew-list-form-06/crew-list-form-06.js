...
function resolveTargetRowCount(savedCount, currentCount, maxRows) {
  ...
  return Math.min(max, Math.max(cur, cur > 0 ? cur : 1));
    ...
    <td class="c9">${ta('', d.joinPort)}</td>
    <td class="c10"><input class="ci" type="text" value="${escAttr(d.joinDate || '')}"${dateIsoAttr(d.joinDateIso)} readonly tabindex="-1"></td>`;
    ...
    function rebuildListForMode(mode) {
      ...
      const savedVar = window._appData.documentOverlay?.crewList?.byType?.[CREW_FORM_06_TYPE];
      HtmlFormListMode.rebuildList({
        ...
        overlayPath: ['crewList', 'byType', CREW_FORM_06_TYPE],
      });
