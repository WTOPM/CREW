const tbody = document.getElementById('tbody');
    ...
    function rebuildListForMode(mode) {
      ...
      const savedVar = window._appData.documentOverlay?.crewList?.byType?.[CREW_FORM_01_TYPE];
      HtmlFormListMode.rebuildList({
        ...
        overlayPath: ['crewList', 'byType', CREW_FORM_01_TYPE],
      });
      if (window.CrewHtmlFormEditorDirty?.captureBaseline) {
    ...
    function navigateBack(feedback) {
      ...
      if (feedback) {
        ...
        target += `${sep}form01Feedback=${feedback}`;
      }
    ...
    const IDENTITY_DOC_LABEL = 'Passport';
    ...
    function loadPositions() {
      ...
      try {
        ...
        if (appData && appData.documentOverlay && appData.documentOverlay.crewList && appData.documentOverlay.crewList.byType) {
          const variant = appData.documentOverlay.crewList.byType[CREW_FORM_01_TYPE];
          if (variant) {
    ...
    async function persistAllChanges() {
      ...
      if (!appData.documentOverlay.crewList) {
        appData.documentOverlay.crewList = { listType: CREW_FORM_01_TYPE, byType: {} };
      }
