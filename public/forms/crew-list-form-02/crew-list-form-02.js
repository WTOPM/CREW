const tbody = document.getElementById('tbody');
    ...
    function navigateBack(feedback) {
      ...
      if (feedback) {
        ...
        target += `${sep}form02Feedback=${feedback}`;
      }
    ...
    const ZOOM_STORAGE_KEY = 'crew02-editor-zoom';
    const ZOOM_MIN = 50;
    const ZOOM_MAX = 200;
    const ZOOM_STEP = 10;
    ...
    const CREW_FORM_02_TYPE = 'type1SeamansBook';
    const IDENTITY_DOC_LABEL = "Seaman's Book";
    const CREW_LIST_MAX_ROWS = 23;
    ...
    function loadPositions() {
      ...
      try {
        ...
        if (appData && appData.documentOverlay && appData.documentOverlay.crewList && appData.documentOverlay.crewList.byType) {
          const variant = appData.documentOverlay.crewList.byType[CREW_FORM_02_TYPE];
          if (variant) {
    ...
    async function persistAllChanges() {
      ...
      if (!appData.documentOverlay.crewList) {
        appData.documentOverlay.crewList = { listType: CREW_FORM_02_TYPE, byType: {} };
      }
      ...
      appData.documentOverlay.crewList.listType = CREW_FORM_02_TYPE;
      appData.documentOverlay.crewList.byType[CREW_FORM_02_TYPE] = {
        ...prev,
    ...
    async function loadAppData() {
      ...
      if (appData) {
        ...
        const idTypeEl = document.getElementById('h-id-type');
        if (idTypeEl) idTypeEl.textContent = IDENTITY_DOC_LABEL;
        ...
        const savedForm02 = appData.documentOverlay?.crewList?.byType?.[CREW_FORM_02_TYPE];
        if (isArrival) {
          setAD('arrival');
        } else {
          setAD('departure');
        ...
        const footerDateEl = document.getElementById('f-footer-date');
        if (footerDateEl && savedForm02?.footerSignatureDate) {
          footerDateEl.value = savedForm02.footerSignatureDate;
        ...
        crewList.forEach(c => {
          ...
          addRow({
            ...
            pob: c.placeOfBirth || '',
            identity: c.seamansBook || '',
          });
        });
      }

      const savedVar = appData?.documentOverlay?.crewList?.byType?.[CREW_FORM_02_TYPE];
