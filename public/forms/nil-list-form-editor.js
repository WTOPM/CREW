/**
 * NIL List HTML editor shell — save, zoom, overlay persistence.
 */
(function (global) {
  const ZOOM_MIN = 50;
  const ZOOM_MAX = 200;
  ...
  function createEditor(overlayKey, feedbackParam) {
    ...
    function loadPositions() {
      ...
      global._currentPositions = loaded || { stamp: {}, sig: {} };
    ...
    function navigateBack(feedback) {
      ...
      const base = returnRaw ? decodeURIComponent(returnRaw) : '/?nilListSettings=1';
    ...
    function defaultStampCss() {
      return { left: '120mm', top: '220mm', width: '65mm', height: '28mm' };
    }
    ...
    async function toggleStamp(checked) {
      ...
      if (url) {
        ...
        const defaults = defaultStampCss();
        showOverlay(el, url, {
    ...
    async function toggleSignature(checked) {
      ...
      if (url) {
        ...
        const defaults = defaultSignatureCss();
        showOverlay(el, url, {
          left: saved.left || defaults.left,
          top: saved.top || defaults.top,
          width: saved.width || defaults.width,
  ...
  global.NilListFormEditor = { createEditor };
