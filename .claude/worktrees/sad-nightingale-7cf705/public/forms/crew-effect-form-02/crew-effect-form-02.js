...
(function (global) {
  ...
  function freshForm02() {
    const appData = global._appData;
    if (!appData || !CE?.buildForm02FromAppData) return null;
    return CE.buildForm02FromAppData(appData, false, { ignoreOverlay: true });
  }

  /** Live-computed header/footer defaults, used to avoid pinning unedited cells (crew grid is never pinned). */
  function defaultCellValuesMap() {
    const form02 = freshForm02();
    if (!form02) return null;
    return {
      'h-pageNo': form02.pageNo,
      'h-nameOfShip': form02.nameOfShip,
      'h-port': form02.portOfArrivalDeparture,
      'h-date': form02.dateOfArrivalDeparture || '',
      'h-nationality': form02.nationalityOfShip,
      'footer-date': form02.footerDate || '',
      'footer-master': form02.footerMaster,
    };
  }
  ...
  function initCellEditor(overlayVariant) {
    ...
    editor.connectCellEditor({
      ...
      collectValues: () => {
        const values = Cells.collectCellValues(defaultCellValuesMap());
