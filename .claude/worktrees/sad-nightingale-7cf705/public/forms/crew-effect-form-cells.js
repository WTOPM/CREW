...
(function (global) {
  ...
  /**
   * Crew grid cells (`d-*`) are always computed live from the crew/passenger list —
   * never persisted — so the document can never freeze on stale data. Only header/
   * footer cells (which differ from the live-computed defaults) get pinned.
   */
  function collectCellValues(defaults) {
    const scope = pageScope() || root;
    if (!scope) return {};
    const cellValues = {};
    scope.querySelectorAll('input.ci[data-cell-key]').forEach((el) => {
      ...
      if (defaults && Object.prototype.hasOwnProperty.call(defaults, key) && defaults[key] === text) {
        return;
      }
      cellValues[key] = text;
    });
  ...
  /** Crew grid cells (`d-*`) are skipped — they're always freshly applied from live data by applyForm01/02. */
