const tbody = document.getElementById('tbody');
    ...
    <td class="c6a"><input class="ci" type="text" value="${d.identity || ''}" readonly tabindex="-1"></td>
    <td class="c6b"><input class="ci" type="text" value="${d.expiry || ''}"${dateIsoAttr(d.expiryIso)} placeholder="DD.MM.YYYY" readonly tabindex="-1"></td>`;
    ...
    function navigateBack(feedback) {
      ...
      if (feedback) {
        ...
        target += `${sep}paxForm02Feedback=${feedback}`;
      }
    ...
    function loadPositions() {
      ...
      try {
        ...
        if (appData && appData.documentOverlay && appData.documentOverlay[OVERLAY_KEY]) {
    ...
    async function persistAllChanges() {
      ...
      if (!appData.documentOverlay[OVERLAY_KEY]) { appData.documentOverlay[OVERLAY_KEY] = {}; }
    ...
    async function loadAppData() {
      ...
      if (appData) {
        ...
        const footerDateEl = document.getElementById('f-footer-date');
        if (footerDateEl && savedOverlay?.footerSignatureDate) {
          footerDateEl.value = savedOverlay.footerSignatureDate;
        }
        ...
        passengerList.forEach((p) => {
          ...
          addRow({
            ...
            pob: p.placeOfBirth || '',
            identity: p.passport || '',
            expiry: fmtDate(p.passportExpiryDate),
