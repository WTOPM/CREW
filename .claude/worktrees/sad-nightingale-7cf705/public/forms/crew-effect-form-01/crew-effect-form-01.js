...
(function (global) {
  ...
  function freshForm01() {
    const appData = global._appData;
    if (!appData || !CE?.buildForm01FromAppData) return null;
    return CE.buildForm01FromAppData(appData, false, { ignoreOverlay: true });
  }
  ...
  function defaultCellValuesMap() {
    const form01 = freshForm01();
    if (!form01) return null;
    return {
      'h-pageNo': form01.pageNo,
      'h-nameOfShip': form01.nameOfShip,
      'h-nationality': form01.nationalityOfShip,
      'footer-master': form01.footerMaster,
    };
  }
  ...
  function initCellEditor(overlayVariant) {
    ...
    editor.connectCellEditor({
      ...
      collectValues: () => global.CrewEffectFormCells.collectCellValues(defaultCellValuesMap()),
