...
export class PackageRunnerService {
  ...
  /** Current Port of Call from the main screen drives Open/Print. */
  readonly currentPort = computed(() => this.storage.ship().portOfCall);
  readonly currentPackage = computed(() =>
    this.storage.printPackages().find((p) => p.port === this.currentPort()),
  );
  ...
  readonly currentItemCount = computed(() => this.currentItems().length);
