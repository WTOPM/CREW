...
export class PdfCrewListV3SbkPService {
  ...
  private drawFooter(
    ...
    this.drawText(
      page,
      voyageDate,
      CREW_LIST_V3_SBK_P_FOOTER.signatureDate,
      font,
      black,
      textRotate,
    );
  ...
  private drawCrewColumns(
    ...
    const drawCell = (text: string, colIndex: number, field: CrewListV3SbkPColField) => {
      this.drawColWrap(
        page,
        text,
        ...
        black,
        textRotate,
        crewListV3SbkPFieldMaxPt(field),
        CREW_LIST_V3_SBK_P_WRAP_MAX_LINES,
      );
    };
    ...
    crew.forEach((member, colIndex) => {
      ...
      drawCell(this.formatJoiningPort(member.joiningPort), colIndex, 'joiningPort');
      drawCell(formatDisplayDate(member.joiningDate), colIndex, 'joiningDate');
    });
  }
  ...
  private drawColWrap(
    ...
    const value = text.trim();
    if (!value) return;
    const lines = this.wrapCellLines(font, value, CREW_LIST_V3_SBK_P_FONT, maxWidth, maxLines);
  ...
  private async loadTemplate(): Promise<Uint8Array> {
    ...
    if (!res.ok) {
      throw new Error(
        'Crew List v3 SBK/P template not found (public/crew-list-v3-sbk-p-empty.pdf)',
      );
    }
