...
/** Crew Effect 02 — Germany; 2 pages, stamp on page 2 only. */
...
export class PdfCrewEffect02Service {
  ...
  async buildFinalBytes(data: AppData): Promise<Uint8Array> {
    ...
    return this.overlay.applyCrewEffect02Overlay(bytes, data.documentOverlay.crewEffect02);
  }
  ...
  async build(data: AppData): Promise<Uint8Array> {
    ...
    if (!pages.length) {
      throw new Error('Crew Effect 02 template has no pages');
    }
  ...
  private drawCrewRows(
    ...
    for (let i = 0; i < CREW_EFFECT_02_ROW_COUNT; i++) {
      ...
      if (form.nilCigarettes) {
        this.drawTableCell(page, font, black, CREW_EFFECT_NIL_LABEL, CREW_EFFECT_02_COL.cigarettes, y, fontSize);
      }
      if (form.nilCigars) {
        this.drawTableCell(page, font, black, CREW_EFFECT_NIL_LABEL, CREW_EFFECT_02_COL.cigars, y, fontSize);
      }
      if (form.nilSpirits) {
        this.drawTableCell(page, font, black, CREW_EFFECT_NIL_LABEL, CREW_EFFECT_02_COL.spirits, y, fontSize);
      }
      if (form.nilWeapons) {
        this.drawTableCell(page, font, black, CREW_EFFECT_NIL_LABEL, CREW_EFFECT_02_COL.weapons, y, fontSize);
      }
      if (form.nilAmmunition) {
        this.drawTableCell(page, font, black, CREW_EFFECT_NIL_LABEL, CREW_EFFECT_02_COL.ammunition, y, fontSize);
      }
      if (others) {
        this.drawTableCell(
          page,
          ...
          black,
          others,
          CREW_EFFECT_02_COL.others,
          y,
          fontSize,
          CREW_EFFECT_02_COL.othersMaxWidth,
        );
      }
    }
  }

  /** One line only — shrink font, then truncate; never pdf-lib maxWidth wrap. */
