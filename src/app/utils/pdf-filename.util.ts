...
export function crewListForm04PdfFileName(
  shipName: string,
  portOfCall: string,
  voyageDate: string,
  isArrival: boolean,
  ...
  const kind = isArrival ? 'Arrival' : 'Departure';
  return `Crew_List_Form_04_${kind}_${pdfFileToken(shipName, 'ship')}_${pdfFileToken(portOfCall, 'port')}_${pdfFileDate(voyageDate)}.pdf`;
}
